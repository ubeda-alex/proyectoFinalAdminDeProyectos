  ...
  try {
    ...
    const newUser = await prisma.$transaction(async (tx) => {
      ...
      if (role === 'CLIENT') {
        ...
        await tx.artist.create({ data: { userId: user.id, category, portfolio, status: 'pending' } });
      }
      return user;
    });
...
app.post('/api/artist/services', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (!artist) return res.status(404).json({ error: 'Perfil de artista no encontrado.' });
    const newService = await prisma.service.create({ data: { artistId: artist.id, name, description, duration, requirements } });
...
app.delete('/api/artist/services/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (!service) return res.status(404).json({ error: 'Servicio no encontrado o no autorizado.' });
...
// ── PUBLIC CATALOG — SCRUM-27, SCRUM-28, SCRUM-29 ────────────────────────────

app.get('/api/artists', async (req, res) => {
  const { category, location, available } = req.query;
  try {
    const where = {};
    // SCRUM-27: filter by category
    if (category && category !== 'all') {
      where.category = { contains: category };
    }
    // SCRUM-28: filter by location/zone
    if (location && location !== 'all') {
      where.location = { contains: location };
    }
    // SCRUM-29: filter by availability (status active = available)
    if (available === 'true') {
      where.status = 'active';
    }
...
// ── BOOKINGS — SCRUM-30, SCRUM-31, SCRUM-32, SCRUM-33 ────────────────────────

// SCRUM-30: Client sends a booking request
app.post('/api/bookings', authenticateToken, async (req, res) => {
  if (req.user.role !== 'CLIENT')
    return res.status(403).json({ error: 'Solo los clientes pueden enviar solicitudes de contrato.' });

  const { artistId, serviceId, eventDate, message } = req.body;
  if (!artistId)
    return res.status(400).json({ error: 'El ID del artista es obligatorio.' });
...
// SCRUM-31: Artist sees received booking requests
app.get('/api/artist/bookings', authenticateToken, async (req, res) => {
  if (req.user.role !== 'ARTIST')
    return res.status(403).json({ error: 'Solo los artistas pueden ver sus solicitudes.' });
...
// SCRUM-32: Artist accepts or rejects a booking
app.patch('/api/artist/bookings/:id', authenticateToken, async (req, res) => {
  if (req.user.role !== 'ARTIST')
    return res.status(403).json({ error: 'Solo los artistas pueden gestionar sus solicitudes.' });
  ...
  try {
    ...
    if (booking.status !== 'REQUESTED')
      return res.status(400).json({ error: 'Solo se pueden gestionar solicitudes en estado REQUESTED.' });
    ...
    res.json({ message: `Solicitud ${status === 'ACCEPTED' ? 'aceptada' : 'rechazada'} con éxito.`, booking: updated });
  } catch (error) {
    res.status(500).json({ error: 'Error interno del servidor.' });
  }
});

// SCRUM-33: Client sees status of their booking requests
app.get('/api/client/bookings', authenticateToken, async (req, res) => {
  if (req.user.role !== 'CLIENT')
    return res.status(403).json({ error: 'Solo los clientes pueden ver sus solicitudes.' });
